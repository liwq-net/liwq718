...
namespace cocos2d
{
    ...
    public class CCTextureAtlas : CCObject
    {
        ...
        public bool initWithFile(string file, int capacity)
        {
            ...
            Texture texture = CCTextureCache.sharedTextureCache().addImage(file);
            if (texture != null)
            {
        ...
        /// WARNING: Do not reinitialize the TextureAtlas because it will leak memory (issue #706)
        /// </summary>
        public bool initWithTexture(Texture texture, int capacity)
        {
        ...
        public void drawNumberOfQuads(int n, int start)
        {
            ...
            for (int i = start; i < start + n; i++)
            {
                ...
                if (quad != null)
                {
                    vertices.AddRange(quad.getVertices(DirectorProjection.Projection3D).ToList());
