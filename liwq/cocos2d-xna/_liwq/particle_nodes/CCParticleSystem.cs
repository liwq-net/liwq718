...
namespace cocos2d
{
    ...
    */
    public class CCParticleSystem : Node, ICCTextureProtocol
    {
        ...
        */
        ...
        public static Dictionary<string, object> dictionaryWithContentsOfFile(string plistFile)
        {
            Dictionary<string, object> dict = new Dictionary<string, object>();
            System.Xml.Linq.XElement root = System.Xml.Linq.XElement.Parse(File.ReadAllText("Content/" + plistFile));

            var dictRoot = root.Elements("dict");
            string key = "";
            foreach (var keyValues in dictRoot)
            {
                foreach (var kv in keyValues.Elements())
                {
                    if (kv.Name == "key")
                    {
                        key = kv.Value;
                    }
                    else if (kv.Name == "real")
                    {
                        dict.Add(key, double.Parse(kv.Value));
                    }
                    else
                    {
        ...
        */
        public bool initWithFile(string plistFile)
        {
            ...
            m_sPlistFile = plistFile;
            Dictionary<string, object> dict = dictionaryWithContentsOfFile(m_sPlistFile);
	...
	/// @todo _profilingTimer = [[CCProfiler timerWithName:@"particle system" andInstance:self] retain];
        ...
        public void initParticle(CCParticle particle)
        {
            ...
            particle.pos.X *= Director.SharedDirector.ContentScaleFactor;
            particle.pos.Y = m_tSourcePosition.Y + m_tPosVar.Y * ccMacros.CCRANDOM_MINUS1_1();
            particle.pos.Y *= Director.SharedDirector.ContentScaleFactor;
            ...
            startS *= Director.SharedDirector.ContentScaleFactor;
            ...
            else
            {
                ...
                endS *= Director.SharedDirector.ContentScaleFactor;
            ...
            if (m_ePositionType == eParticlePositionType.kCCPositionTypeFree)
            {
                ...
                particle.startPos = CCPointExtension.ccpMult(p, Director.SharedDirector.ContentScaleFactor);
            }
            else if (m_ePositionType == eParticlePositionType.kCCPositionTypeRelative)
            {
                particle.startPos = CCPointExtension.ccpMult(_position, Director.SharedDirector.ContentScaleFactor);
            }

            // direction
            float a = CCUtils.CC_DEGREES_TO_RADIANS(m_fAngle + m_fAngleVar * ccMacros.CCRANDOM_MINUS1_1());
            ...
            if (m_nEmitterMode == (int)eParticleMode.kCCParticleModeGravity)
            {
                ...
                s *= Director.SharedDirector.ContentScaleFactor;
                ...
                particle.modeA.radialAccel *= Director.SharedDirector.ContentScaleFactor;

                // tangential accel
                particle.modeA.tangentialAccel = modeA.tangentialAccel + modeA.tangentialAccelVar * ccMacros.CCRANDOM_MINUS1_1();
                particle.modeA.tangentialAccel *= Director.SharedDirector.ContentScaleFactor;
            }
            ...
            else
            {
                ...
                startRadius *= Director.SharedDirector.ContentScaleFactor;
                endRadius *= Director.SharedDirector.ContentScaleFactor;
                ...
                particle.modeB.degreesPerSecond = CCUtils.CC_DEGREES_TO_RADIANS(modeB.rotatePerSecond + modeB.rotatePerSecondVar * ccMacros.CCRANDOM_MINUS1_1());
            }
        }
	...
	/// @todo CCProfilingBeginTimingBlock(_profilingTimer);
            ...
            if (m_ePositionType == eParticlePositionType.kCCPositionTypeFree)
            {
                ...
                currentPosition.X *= Director.SharedDirector.ContentScaleFactor;
                currentPosition.Y *= Director.SharedDirector.ContentScaleFactor;
            }
            else if (m_ePositionType == eParticlePositionType.kCCPositionTypeRelative)
            {
